...
final routerProvider = Provider<GoRouter>((ref) {
  ...
  return GoRouter(
    ...
    redirect: (context, state) {
      ...
      final VersionCheckState versionState = ref.read(versionCheckProvider);
      final bool isSplashRoute = state.matchedLocation == '/splash';
      final bool isForceUpdateRoute = state.matchedLocation == '/force-update';

      // Hold on the splash screen until the stored session and the public
      // version check have both finished, so the app never flashes login
      // before a required update (or before the session is known).
      if (authState.isInitializing || versionState.isChecking) {
        return isSplashRoute ? null : '/splash';
      }

      // Below minimum_version the user cannot go any further.
      if (versionState.decision == VersionUpdateDecision.required) {
        return isForceUpdateRoute ? null : '/force-update';
      }

      if (isSplashRoute || isForceUpdateRoute) {
        return authState.isAuthenticated ? '/feed' : '/login';
      }
      ...
      final bool isPublicAuthRoute = isLoginRoute || state.matchedLocation == '/forgot-password';
...
final routerListenableProvider = Provider<ChangeNotifier>((ref) {
  ...
  ref.listen(versionCheckProvider, (_, __) => notifier.notify());
  return notifier;
});
