...
class _MyJobsScreenState extends ConsumerState<MyJobsScreen> {
  ...
  Widget _buildJobCard(JobModel job) {
    final l10n = AppLocalizations.of(context)!;
    final bool isDraft = job.status == 'pending' || job.status == 'draft';
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ClipRRect(
                  ...
                  child: job.image != null && job.image!.isNotEmpty
                      ? Image.network(
                          job.image!.fullImageUrl,
                          width: 52,
                          height: 52,
                          fit: BoxFit.cover,
                          errorBuilder: (context, error, stackTrace) => _jobCardLogoFallback(),
                        )
                      : _jobCardLogoFallback(),
                ),
            ...
            Wrap(
              ...
              runSpacing: 8,
              children: [
                _buildSmallIconText(LucideIcons.mapPin, job.location),
                _buildSmallIconText(LucideIcons.briefcase, _translateJobType(job.jobType)),
                _buildSmallIconText(LucideIcons.calendar, _formatDate(job.createdAt)),
              ],
            ),
            const SizedBox(height: 20),
            Wrap(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              leading: const Icon(LucideIcons.pencil, color: Color(0xFF10B981)),
              title: Text(
                l10n.jobActionEdit,
  ...
  Widget _jobCardLogoFallback() {
    return Container(
      ...
      child: const Icon(LucideIcons.briefcase, color: AppColors.primary, size: 26),
    );
  }
  ...
  Widget _buildStatsSection(dynamic state) {
    ...
    final draftCount = state.jobs.where((j) => j.status == 'pending' || j.status == 'draft').length;
